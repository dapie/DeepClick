...
router.post("/auth/login/", async function(req, res, next) {
	...
	const cryptoPass = crypto
		.createHash("sha1")
		.update(JSON.stringify(secret + password))
		.digest("hex");
	let findedUser = Realm.objects("User").filtered(
		'email = "' + email + '"'
	)[0];
...
router.get("/db/links", async function(req, res, next) {
	let findedLinks = Realm.objects("Link")
		.filtered('ownerID = "' + req.user.id + '"')
		.sorted("id");
	res.json(findedLinks);
});

router.get("/db/links/:id", async function(req, res, next) {
	let findedLink = Realm.objects("Link").filtered(
		'id = "' + req.params.id + '"'
	)[0];
...
router.post("/db/links/add", async function(req, res, next) {
	...
	let linkOnServer = Realm.objects("Link").filtered(
		'link = "' + link + '"'
	)[0];
	if (linkOnServer)
...
router.post("/db/links/click", async function(req, res, next) {
	...
	if (!linkOnServer) return res.status(400).send("Диплинк не существует");
	Realm.write(() => {
		linkOnServer.clicks++;
	});
	res.json({ status: "OK" });
...
router.post("/db/links/delete", async function(req, res, next) {
	...
	let linkOnServer = Realm.objects("Link").filtered('id = "' + id + '"')[0];
	if (!linkOnServer)
		return res.status(400).send("Такой диплинк уже не существует");
	if (linkOnServer.ownerID != req.user.id)
		return res.status(400).send("Ссылка не пренадлежит вам");
	Realm.write(() => {
